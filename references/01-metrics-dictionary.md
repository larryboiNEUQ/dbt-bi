...
指标字典是**唯一权威表**：业务口径与 PBI 语义建模写在**同一张表。**

- **改口径** → 先改业务列
- **进 PBI** → 读同一行的建模列 + schema 的 grain/键（落地见 §4.2 / references/04-pbi-pbip.md）
...
| 维度 | 按什么切、看什么属性？ | **维度目录（schema.yml 侧）** + 字典 `allowed_dims` |
